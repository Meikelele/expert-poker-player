...
class LoadedDQNCheckpoint:
    """Wczytany checkpoint modelu DQN."""
...
def save_dqn_checkpoint(
    ...
    *,
    ...
    """Zapisuje model DQN wraz z metadanymi eksperymentu."""
    ...
    checkpoint: dict[str, object] = {
        ...
        "input_size": policy_network.input_size,
        "hidden_sizes": list(
            policy_network.hidden_sizes
        ),
        "state_representation": (
...
def load_dqn_checkpoint(
    ...
    *,
    ...
    """Wczytuje model DQN wraz z metadanymi eksperymentu."""
    ...
    state_representation = StateRepresentation(
        checkpoint["state_representation"]
    )

    reward_type = RewardType(
        checkpoint["reward_type"]
    )
    ...
    config_values: dict[str, Any] = dict(
        cast("dict[str, Any]", checkpoint["config"])
    )

    if not isinstance(
        config_values.get("hidden_sizes"),
        list,
    ):
        raise ValueError(
            "checkpoint hidden_sizes must be a list"
        )

    config_values["hidden_sizes"] = tuple(
        config_values["hidden_sizes"]
    )
    ...
    if not isinstance(training_seed, int):
        raise ValueError(
            "checkpoint training seed must be an integer"
        )
    ...
    if not isinstance(input_size, int):
        raise ValueError(
            "checkpoint input size must be an integer"
        )
    ...
    if not isinstance(
        ...
        raise ValueError(
            "checkpoint architecture must be a list"
        )

    stored_hidden_sizes = tuple(
        cast("list[int]", checkpoint["hidden_sizes"])
    )
    ...
    if input_size != encoder.output_size:
        raise ValueError(

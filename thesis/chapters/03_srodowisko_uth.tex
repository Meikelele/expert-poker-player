...
rozdanie na riverze bez showdownu. Zależności te zestawiono w
tabeli~\ref{tab:fazy-akcje} i przedstawiono na
rysunku~\ref{fig:maszyna-stanow-uth}.

\begin{table}[htbp]
    \centering
    \caption{Fazy, legalne akcje i przejścia środowiska UTH}
    ...
    \begin{tabular}{
        ...
        \texttt{CHECK}, \texttt{BET\_3X}, \texttt{BET\_4X}
        &
        \texttt{CHECK} spala kartę, odkrywa flop i przechodzi do
        \texttt{FLOP}; zakład \texttt{BET} odkrywa cały stół, prowadzi do
        showdownu i \texttt{TERMINAL}.
        \\
        \hline
        ...
        \texttt{CHECK}, \texttt{BET\_2X}
        &
        \texttt{CHECK} odkrywa turn i river i przechodzi do \texttt{RIVER};
        \texttt{BET\_2X} prowadzi do showdownu i \texttt{TERMINAL}.
        \\
        \hline
        ...
        \texttt{BET\_1X}, \texttt{FOLD}
        &
        \texttt{BET\_1X} prowadzi do showdownu, a \texttt{FOLD} kończy
        rozdanie bez showdownu; oba przejścia prowadzą do \texttt{TERMINAL}.
        \\
        \hline
        ...
        brak
        &
        Stan końcowy z wynikiem i rozliczeniem; kolejne rozdanie wymaga
        wywołania \texttt{reset()}.
        \\
        \hline
    \end{tabular}
\end{table}
...
późniejszej fazy. Najdłuższa ścieżka zawiera trzy decyzje
...
natomiast najkrótsza jedną, po zakładzie \(3\times\) lub \(4\times\) przed
flopem. Zastosowanie osobnych akcji \texttt{BET\_1X}, \texttt{BET\_2X},
\texttt{BET\_3X} i \texttt{BET\_4X} zamiast jednej ogólnej akcji
\texttt{BET} wiąże mnożnik Play jednoznacznie z fazą i eliminuje możliwość
przekazania niepoprawnej wysokości zakładu.

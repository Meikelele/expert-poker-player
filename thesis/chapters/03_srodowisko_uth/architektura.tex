...
Wartości zakładów wyrażono w~jednostkach względnych, tzn. jedna jednostka
...
wykonania i~może wynosić od jednej do czterech jednostek. Przyjęto takie
podejście, aby uniezależnić środowisko od konkretnej waluty i~wysokości stawki.

Silnik zwraca szczegółowe rozliczenie poszczególnych zakładów oraz łączny
wynik netto rozdania, będący podstawą nagrody terminalnej. Rozdzielono mechanizm
rozliczania gry od funkcji nagrody. Uzasadnienie tej decyzji przedstawiono
w~podrozdziale~\ref{sec:funkcja-nagrody}.

Zadbano również o~powtarzalność eksperymentów, przez co
talia kart może być tasowana przy użyciu ziarna generatora liczb
pseudolosowych. Dzięki temu możliwe jest odtworzenie całej sekwencji rozdań.
W~testach jednostkowych zastosowano dodatkowo talię deterministyczną,
...
Przyjęto taki podział, aby ograniczyć zależności pomiędzy poszczególnymi częściami
...
\texttt{UTHGame} pełni rolę menedżera tej warstwy. Na podstawie aktualnego
...
pozwala rozgraniczyć logikę środowiska od implementacji agenta.

W~tabeli~\ref{tab:moduly-srodowiska} przedstawiono najważniejsze moduły

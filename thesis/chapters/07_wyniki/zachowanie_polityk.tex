...
Według Volodmyra Mnih'a i współautorów, tego rodzaju przedwczesna zbieżność do suboptymalnych,
deterministycznych rozwiązań jest częstym problemem metod policy gradient, a jednym ze sposobów jej ograniczenia jest dodanie
regularyzacji entropii polityki do funkcji celu \cite{mnih_a3c_2016}.
...
regularyzacji entropii, ani baseline'u, co zwiększa podatność tej
metody na opisywane zjawisko.
...
końcowej ewaluacji, niezależnie od obserwowanego stanu. Dla żadnej
...
Na osiemnaście spośród dwudziestu przebiegów REINFORCE degeneracja
do jednej, niezależnej od stanu akcji nie jest więc wyjątkiem
dotyczącym pojedynczej konfiguracji, lecz najczęstszym wynikiem
całego eksperymentu. Co istotne, konfiguracja REINFORCE FEATURES ze
...
składają się z wielu kolorów i faz rozdania, natomiast dwa są
jednokolorowe na całej długości.
...
podałem, jaki odsetek wszystkich rozdań do nich dotarł. REINFORCE
dociera do riveru w zaledwie 9\% rozdań, podczas gdy dla DQN odsetek
ten wynosi 54\%, co jest bezpośrednią konsekwencją częstego wyboru
najwyższego zakładu już przed flopem.
...
Pokusiłem się również o analizę zachowania agentów poprzez porównanie decyzji przed
...
kolor pola oznacza dominującą akcję wybraną dla danego układu.
